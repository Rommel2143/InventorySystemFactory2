...
Public Class FG_IN_Results
    ...
    Private Sub dtpicker_ValueChanged(sender As Object, e As EventArgs) Handles dtpicker.ValueChanged
        Try
            con.Close()
            con.Open()
            Dim cmdselect As New MySqlCommand("Select distinct `batch` FROM `f2_fg_scan`
    ...
    Private Sub cmbbatchout_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbbatchin.SelectedIndexChanged
        Try
            con.Close()
            con.Open()
            Dim cmdrefreshgrid As New MySqlCommand("SELECT `qrcode`,`partcode`,`lotnumber`, `remarks`, `qty` FROM `f2_fg_scan`
            ...
            con.Close()
            con.Open()
            Dim cmdrefreshgrid2 As New MySqlCommand("SELECT f2_fg_scan.`partcode` AS Partcode, f2_fg_masterlist.`partname`, SUM(`qty`) AS TOTAL FROM `f2_fg_scan`
                                                   JOIN `f2_fg_masterlist` ON `f2_fg_scan`.`partcode`=`f2_fg_masterlist`.`partcode`
